...
define :mongodb_instance,
  ...
  new_resource.replicaset_members         = node['mongodb']['replicaset_members']
  new_resource.replicaset_name            = node['mongodb']['config']['replSet']
  new_resource.port                       = node['mongodb']['config']['port']
  new_resource.auth_user                  = node['mongodb']['auth_user']
  new_resource.auth_password              = node['mongodb']['auth_password']
  ...
  if new_resource.auto_configure_replicaset && (not new_resource.replicaset_members.empty?)
    raise Chef::Exceptions::ConfigurationError,
          'Both [:mongodb][:auto_configure][:replicaset] and [:mongodb][:replicaset_members] are set in JSON. Please use only one of option.'
  end
  ...
  # external replicaset
  if new_resource.is_replicaset && (not new_resource.replicaset_members.empty?) 
    ruby_block 'config_external_replicaset' do
      block do
        MongoDB.add_node_to_external_replicaset(node,
                                                new_resource.replicaset_members,
                                                replicaset_name,
    ...
    ruby_block 'run_config_external_replicaset' do
      block {}
      action :run
    end
  end
